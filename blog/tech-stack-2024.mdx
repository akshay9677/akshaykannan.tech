...
bully you into using one and the one I've adopted is [Next.js](https://nextjs.org/). Have been using page router for a few
months recently moved to the App router and I have some mixed opinions on it.

### Next.js - App router

First of all, if you've been using page router extensively, there is some serious learning curve here. We won't have to use
`getServerSideProps` and `getStaticProps` anymore, which is a pretty great thing ;-) . And it is a lot fun now to use react
server components. I've discussed a few features of app router below,
...
functionalities could remain client-side. Additionally, server components eliminate the complexity associated with managing
useEffect hooks. `useEffect` can be tricky to manage. RSC simplifies things by eliminating the need for it entirely.
...
Next.js App Router's layouts are a game-changer! Previously, I'd create separate layouts for different page types and import
them into each page. Now, a single layout.js file at the root can define the layout structure, simplifying its application
across various routes.

#### 3) File based routing

While file-based routing offers simplicity, it can introduce maintainability challenges. Having multiple files with the
same name (e.g., page.tsx, metadata.tsx) makes searching difficult. Additionally, implementing dynamic routes with parameters
in between routes (like /products/:productId/details) can become difficult, potentially requiring extra folders. Despite these
drawbacks, file-based routing can be a viable option.

<br />
It's a strong option overall, also I've seen a few people complain about the development
performance, it is the same as page router for me, nothing much to complain about
it.
