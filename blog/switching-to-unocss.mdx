...
I write about them if they are worth writing about, one such thing is Unocss. This post will be a deep dive on why I am
moving to unocss.
...
their docs I came across a concept they had called `Purging` which can be enabled by adding `purge:true` in their config,
...
your app's performance because of unecessary computations (we are generating the css and are not using them). But
...
(even in Windi CSS). To do so I had to override their config with a butt load of information, which was a lot to do for simple functionalities.
