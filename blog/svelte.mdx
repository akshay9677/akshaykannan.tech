...
[Angular](https://angular.io/) have been the top contenders. Svelte a new framework has been gaining a lot of attention in
the recent days. There are a lot of interesting things in Svelte. Learn more on why svelte stands out from the existing list
of frameworks.
...
work with svelte. To learn about no virtual dom in svelte, lets see what is a virtual dom.
...
As you can see react and vue rely on an API which compares virtual DOM and actual DOM to apply reactivity. So this api should
be there in our bundled code for reactivity to work. Without the api there is no way of knowing when to update the virtual dom.
Because of this API a lot of framework code might be injected into the browser, which might slow your apps performance compared
to a framework without this api.

:::tip note
Virtual DOM is **fast** but with few limitations. As it helps developers to concentrate on more creative tasks rather than
focussing on reactivity and performance, it is used in lot of frameworks despite the limitations.
:::

### Svelte - No Virtual DOM

Svelte is such a framework that does not use Virtual DOM. Svelte,rather than maintaining a virtual DOM, compiles the code to tiny
frame less vanilla JavaScript bundle, which updates DOM whenever state changes. This means the reactivity runs on the compiler,
instead of relying on an API to do so. Therefore no overhead framework code is injected into our browser, this also means Svelte
will not be present in our production build, thus it is called as a "disappearing framework". This increases app's performance
to a great extent.
