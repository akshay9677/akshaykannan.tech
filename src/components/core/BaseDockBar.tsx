...
function DockCard({ children, id, handleClick }: DockCardProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-1" key={id}>
      ...
      <motion.button
        ...
        className="rounded-lg border aspect-square bg-container-secondary saturate-90 brightness-90 transition-filter duration-50 hover:saturate-100 hover:brightness-112 border-border-primary"
        onClick={() => {
          if (handleClick) {
            handleClick();
          }
        }} // Click handler to start/stop animation
        style={{
...
function DockDivider() {
  return (
    ...
    >
      <span className="w-0.5 h-full rounded bg-border-primary"></span>
    </motion.div>
  );
}
